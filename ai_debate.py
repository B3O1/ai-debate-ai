...
# ==========================================
def create_debate_prompt(user_claim, personality, attitude, atmosphere, topic, background, goal, condition, history_text):
    ...
    full_prompt = (
        ...
        "4. [극단적 채점]: 논리성과 설득력 점수(0~100)를 대충 중간 점수(50점)로 주지 마세요. 완벽하면 90점 이상, 억지나 논리적 오류가 있으면 가차 없이 30점 이하로 극단적으로 평가하세요.\n"
        f"{style_rules}\n\n"
        f"[요약본 히스토리]\n{history_text}\n"
...
# ==========================================
# 💡 [4. 메인 토론 파이프라인 (백엔드 HP 관리 추가)]
# ==========================================
async def run_debate_pipeline(user_claim, model_type, personality, attitude, atmosphere, topic, background, goal, condition, db: Session, session_string_id: str):
    global model_token_usage

    db_session = db.query(DebateSession).filter(DebateSession.session_string_id == session_string_id).first()
    if not db_session:
        # DB 세션 새로 생성 시 기본 점수 30점으로 세팅
        db_session = DebateSession(session_string_id=session_string_id, model_type=model_type, atmosphere=atmosphere)
        if hasattr(db_session, 'current_score'):
            db_session.current_score = 30
        db.add(db_session)
        db.commit()
    ...
    full_prompt = create_debate_prompt(user_claim, personality, attitude, atmosphere, topic, background, goal, condition, history_text)
    ...
    try:
        if model_type == "groq" and groq_client:
            ...
            raw1 = res1.choices.message.content
            ...
            raw_response = res2.choices.message.content
    ...
    if result:
        ...
        # 💡 [백엔드 점수 계산 및 DB 업데이트 로직]
        eval_data = result.get('evaluation', {})
        logic_score = eval_data.get('logic_score', 50)
        persuasion_score = eval_data.get('persuasion_score', 50)
        total_turn_score = logic_score + persuasion_score

        # 구간별 점수 증감폭 계산
        if total_turn_score <= 50:
            score_delta = -5
        elif total_turn_score <= 75:
            score_delta = -3
        elif total_turn_score >= 175:
            score_delta = 5
        elif total_turn_score >= 150:
            score_delta = 3
        else:
            score_delta = 0

        # DB의 current_score 업데이트
        current_val = getattr(db_session, 'current_score', 30)
        if current_val is None:
            current_val = 30
            
        new_score = current_val + score_delta
        
        # 최소/최대 점수 제한이 필요하다면 추가 (예: 0~100)
        if new_score < 0: new_score = 0
        if new_score > 100: new_score = 100
            
        if hasattr(db_session, 'current_score'):
            db_session.current_score = new_score

        # 프론트엔드로 전달할 평가 데이터에 계산 결과 추가
        result['evaluation']['score_delta'] = score_delta
        result['evaluation']['current_score'] = new_score

        # 메시지 저장
        user_msg = Message(session_id=db_session.id, role="user", content=user_claim, summary=result.get('user_summary', ''))
        ai_msg = Message(session_id=db_session.id, role="ai", content=result.get('ai_rebuttal', ''), summary=result.get('ai_summary', ''))
...
async def run_evaluation_pipeline(db: Session, session_string_id: str):
    ...
    # 💡 DB에서 관리한 최종 점수를 꺼내옵니다.
    final_score = getattr(db_session, 'current_score', 30)
    if final_score is None:
        final_score = 30

    # 💡 꺼내온 최종 점수를 프롬프트에 확정값으로 주입합니다.
    prompt = (
        f"당신은 냉철한 토론 심판입니다. 사용자의 토론 결과, 최종 점수는 **{final_score}점**으로 확정되었습니다.\n"
        f"아래 대화 기록을 분석하여, 왜 이 점수({final_score}점)를 받게 되었는지 타당한 근거와 장단점을 JSON으로 작성하세요.\n"
        f"반드시 다음 구조를 지키세요. (score 값은 무조건 {final_score}로 고정하세요):\n"
        f'{{"score": {final_score}, "logic_score": 0~100, "persuasion_score": 0~100, '
    ...
    try:
        if cohere_client:
            ...
            if result:
                ...
                # 혹시라도 AI가 score를 다른 값으로 바꿨을 경우를 대비해 다시 덮어씌웁니다.
                result['score'] = final_score
                return result
        return {"score": final_score, "feedback": "심판 호출 실패"}
    except Exception as e:
        print(f"Eval Error: {e}")
        return {"score": final_score, "feedback": f"에러 발생: {str(e)}"}
